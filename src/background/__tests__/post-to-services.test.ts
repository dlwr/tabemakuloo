...
describe('postToServices', () => {
	it('posts to every requested service', async () => {
		const a = new FakeService('A');
		const b = new FakeService('B');
		await postToServices(postData, ['a', 'b'], {a, b});
		expect([a.posted.length, b.posted.length]).toEqual([1, 1]);
	});
	...
	it('returns a result per service in the requested order', async () => {
		const results = await postToServices(postData, ['b', 'a'], {a: new FakeService('A'), b: new FakeService('B')});
		expect(results.map(result => result.service)).toEqual(['B', 'A']);
	});
