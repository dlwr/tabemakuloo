export type PostKind = 'quote' | 'photo' | 'reblog' | 'link';
export type ServiceId = 'tumblr' | 'hatena' | 'x';
...
export const postKinds: PostKind[] = ['quote', 'photo', 'reblog', 'link'];
...
export async function loadDefaultDestinations(storage: StorageArea): Promise<DefaultDestinations> {
	...
	return {
		quote: supportedOnly('quote'), photo: supportedOnly('photo'), reblog: supportedOnly('reblog'), link: supportedOnly('link'),
	};
}
